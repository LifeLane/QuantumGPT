...
const CryptoScreenerOutputSchema = z.object({
  results: z.array(
    z.object({
      ...
      price: z.number().describe('The current price of the cryptocurrency, obtained using the market data tool.'),
      volume: z.number().describe('The 24h trading volume of the cryptocurrency, obtained using the market data tool.'),
...
If the tool returns null or fails for a symbol, you can state that live data couldn't be fetched for it but still include it if it strongly matches other criteria, noting the data absence.

Present the results in a structured format. For each cryptocurrency, include:
- its symbol
- a concise summary of why it meets the criteria
- its current price (from the tool)
- its 24h volume (from the tool)
